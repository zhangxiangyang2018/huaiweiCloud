<template>
  ...
  <div class="file">
      ...
      <div class="content">
        ...
        -webkit-box-orient: vertical;">{{item.name}}</div>
            ...
            <ul
              ...
              @mouseleave="hideMenu()"
            >
              <li v-if="item.type==0&&userId!=0" @click="subscribe(item)">订阅</li>
    ...
    <!-- 属性列表 -->
    <el-dialog title="提示" :visible.sync="isAttribute" width="40%">
...
export default {
  ...
  methods: {
    ...
    subscribe(item) {
      let that = this
      let params = {
        userId: JSON.parse(window.sessionStorage.getItem('userInfor')).userId,
        menuId: item.id
      }
      this.$httpPost('public/subscribe', params).then(function(res) {
        if (res.data.code == 0) {
          that.$message({
    ...
    seeAttr(item) {
      ...
      this.$httpPost('public/meta', params).then(function(res) {
        if (res.data.code == 0) {
          that.attrData = []
    ...
    fileUpload(id, ev) {
      ...
      this.$httpUpload('public/upload', form, config)
        .then(res => {
    ...
    search() {
      let that = this
      let params = 'keyWord=' + this.keyWords
      if (this.keyWords.length) {
        this.$httpGet('public/search', params).then(function(res) {
          if (res.data.code == 0) {
    ...
    addSure() {
      ...
      //console.log(  params)
      this.$httpPost('public/add', params).then(function(res) {
        if (res.data.code == 0) {
          that.init()
          that.isAdd = false
          that.backState = false
    ...
    init() {
      ...
      this.$httpGet('public/fileTree').then(function(res) {
