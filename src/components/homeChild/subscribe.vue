...
export default {
  ...
  methods: {
    ...
    cancel(item){
      ...
      let params = {
        menuId:item.id,
        subscribeUserId: JSON.parse(window.sessionStorage.getItem('userInfor')).userId
      }
    ...
    search() {
      let that = this
      let params = 'keyWord=' + this.keyWords +'&ower='+   JSON.parse(window.sessionStorage.getItem('userInfor')).userId
...
// //深拷贝原对象
      ...
      let that = this
      let params ={
          ower: JSON.parse(sessionStorage.getItem("userInfor")).userId
      }
